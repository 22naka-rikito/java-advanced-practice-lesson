...
<body>
	...
	<h2>現在の登録情報</h2>
	<div class="info">
		<label>名前：</label> ${latestCar.getCarName()}<br> <label>車体の色：</label>
		${latestCar.getBodyColor()} <br> <label>最高速度：</label>
		${latestCar.getMaxSpeed()} <br> <label>現在速度：</label>
		${latestCar.getSpeed()} <br>
	</div>
	...
	<form action="updateServlet" method="post">
		<div class="info">
			<label>車体の色：</label> <input type="text" class="width-50"
				name="bodyColor" value="${car.getBodyColor()}"> <br> <label>現在速度：</label>
			<input type="number" class="width-50" name="speed"
				value="${car.getSpeed()}"> <br>
			<button type="submit" name="btn" value="update">変更</button>
			<br>
		</div>
